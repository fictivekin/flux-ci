...
class User(db.Entity):
    ...
    def get_by_login_details(cls, user_name, password):
        ...
        return orm.select(
            x for x in cls if x.name == user_name and x.passhash == passhash
        ).first()
...
class Build(db.Entity):
  ...
  """
    ...
    def path(self, data=Data_BuildDir):
        base = os.path.join(
            config.build_dir, self.repo.name.replace("/", os.sep), str(self.num)
        )
        if data == self.Data_BuildDir:
            ...
            return os.path.join(
                config.override_dir, self.repo.name.replace("/", os.sep)
            )
        else:
                ...
                try:
                    resp = requests.post(
                        ...
                        json={
                            ...
                            "target_url": url_for(
                                "view_build",
                                path=posixpath.join(build.repo.name, str(build.num)),
                                _external=True,
                            ),
